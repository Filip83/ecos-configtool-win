...
bool ecSettings::LoadConfig()
{
...
#ifdef __WXMSW__
    {
        ...
        if (!versionKey.IsEmpty() && !config2.Read(wxString(wxT("Default Build Tools Path")),  strDefaultBuildToolsPath).IsEmpty())
        {
#ifdef __WXMSW__
...
wxString ecSettings::GetCygwinInstallPath()
{
    ...
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"Software\\Cygnus Solutions\\Cygwin\\mounts v2\\/", 0, KEY_READ, &hKey)) {
        if (ERROR_SUCCESS == RegQueryValueEx(hKey, L"native", NULL, & type, value, & sz)) {
    ...
    hKey = 0;
    sz = sizeof(value);
    if (strCygwinInstallPath.IsEmpty() && (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Cygnus Solutions\\Cygwin\\mounts v2\\/", 0, KEY_READ, &hKey))) {
        if (ERROR_SUCCESS == RegQueryValueEx(hKey, L"native", NULL, & type, value, & sz)) {
            strCygwinInstallPath = (const char*) value;
        }
        RegCloseKey(hKey);
...
const ecFileName& ecSettings::DefaultExternalBrowser()
{
    ...
    if(bFirstTime){
        ...
        {
			//wxStringBuffer tmp(m_strDefaultExternalBrowser, MAX_PATH);
			WCHAR tmp[MAX_PATH];
            f.Close();
            bool rc=((int)  ::FindExecutable(strFile,wxT("."),tmp)>32);
            m_strDefaultExternalBrowser = tmp;
            if(!rc){
