...
const ecFileName ecFileName::ShortName() const
{
    ...
    if(dwSize>0){
        //wxString strCopy;
		LPWSTR strCopy = new WCHAR[dwSize+1];
        ::GetShortPathName (*this, strCopy, 1+dwSize);
        //strCopy.UngetWriteBuf();
        return strCopy;
    } else {
        return *this;
...
#ifdef __WXMSW__
	WCHAR rc[1 + MAX_PATH];
    bool b=(TRUE==PathRelativePathTo(rc,current,FILE_ATTRIBUTE_DIRECTORY,compare,0));
...
const ecFileName ecFileName::CygPath () const 
{
    ...
    if(wxIsalpha(rc[(size_t)0]) && wxTChar(':')==rc[(size_t)1])
    {
        ...
        wxString s = wxString(wxT("/mnt/")) + wxString(rc[(size_t)0]) + rc.Mid(2);
        rc = s;
    }
    size_t i;
    for (i = 0; i < rc.Len(); i++)
