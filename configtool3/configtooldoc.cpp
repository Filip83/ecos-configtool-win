...
bool ecConfigToolDoc::OnSaveDocument(const wxString& filename)
{
    ...
    { // errors already emitted
        ...
        */
        ...
        try
        {
			wxString tmp = filename.ToUTF8();
            m_CdlConfig->save (tmp.ToStdString());
            rc=TRUE;
        }
            ...
            */
            ...
            if (!wxGetApp().GetSettings().m_editSaveFileOnly)
            {
                ...
                else if (GenerateHeaders() && CopyMLTFiles())
                {
					...
					rc = generate_build_tree(GetCdlConfig(), m_strBuildTree.CygPath().ToStdString(), m_strInstallTree.CygPath().ToStdString());
                    rc = TRUE;
                }
            }
            else
...
bool ecConfigToolDoc::GenerateBuildTree()
{
    ...
    if (CanGenerateBuildTree())
    {
        ...
        else if (GenerateHeaders() && CopyMLTFiles())
        {
			...
			bool rc = generate_build_tree(GetCdlConfig(), m_strBuildTree.CygPath().ToStdString(), m_strInstallTree.CygPath().ToStdString());
            rc = TRUE;
        }
...
bool ecConfigToolDoc::UpdateBuildInfo(bool WXUNUSED(bFirstTime))
{
    try {
		...
		generate_build_tree(GetCdlConfig(), GetBuildTree().ToStdString(), GetInstallTree().ToStdString());
        return TRUE;
    }
    catch(...){
        return FALSE;
