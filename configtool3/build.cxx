...
#if ECOS_USE_CYGDRIVE == 1
	wxString tmp = cygpath(install_tree.c_str());
	//fprintf(stream, "export PREFIX := %s\n", cygpath(install_tree.c_str()).c_str()); // double conversion to force /cygdrive/c format
	tmp = tmp.ToUTF8();
	fprintf(stream, "export PREFIX := %s\n", tmp.ToStdString().c_str()); // double conversion to force /cygdrive/c format
#else
	fprintf(stream, "export PREFIX := %s\n", install_tree.c_str());
#endif
	//fprintf (stream, "export PREFIX := %s\n", install_tree.c_str ());
...
#if ECOS_USE_CYGDRIVE == 1
	tmp = cygpath(nativepath(info.repository));
	tmp = tmp.ToUTF8();
