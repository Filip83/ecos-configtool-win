...
//#warning "native Win32 build does not support UNC file paths"
  ...
  if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Cygnus Solutions\\Cygwin\\mounts v2", 0, KEY_READ, &hKey)) {
    if (ERROR_SUCCESS == RegQueryValueEx(hKey, L"cygdrive prefix", NULL, & type, value, & sz)) {
      str = String::CStrToUnicodeStr((const char*)value);
    }
    RegCloseKey(hKey);
  }
  ...
  hKey = 0;
  sz = sizeof(value);
  if (str.empty() && (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"Software\\Cygnus Solutions\\Cygwin\\mounts v2", 0, KEY_READ, &hKey))) {
    if (ERROR_SUCCESS == RegQueryValueEx(hKey, L"cygdrive prefix", NULL, & type, value, & sz)) {
      str = String::CStrToUnicodeStr((const char*)value);
    }
    RegCloseKey(hKey);
